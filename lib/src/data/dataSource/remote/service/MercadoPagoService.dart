...
class MercadoPagoService {
  ...
  Future<Resource<List<MercadoPagoIdentificationType>>>
    ...
    try {
      Uri url = Uri.https(
          ApiConfig.API_ECOMMECE, '/mercadopago/identification_types');
  ...
  Future<Resource<MercadoPagoCardTokenResponse>> createCardToken(
    ...
    try {
      Uri url = Uri.https(ApiConfig.API_ECOMMECE, '/mercadopago/card_token');
      Map<String, String> headers = {
        "Content-Type": "application/json",
        "Authorization": await token
      };
  ...
  Future<Resource<MercadoPagoPaymentResponse2>> createPayment(
    ...
    try {
      Uri url = Uri.https(ApiConfig.API_ECOMMECE, '/mercadopago/payments');
      final uuid = Uuid();
      Map<String, String> headers = {
        "Content-Type": "application/json",
        "Authorization": await token,
  ...
  Future<Resource<MercadoPagoInstallments>> getInstallments(
    ...
    try {
      Uri url = Uri.https(ApiConfig.API_ECOMMECE,
