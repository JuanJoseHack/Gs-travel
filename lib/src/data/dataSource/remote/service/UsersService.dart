...
class UsersService {
  ...
  Future<Resource<User>> update(int id, User user) async {
    try {
      ...
      Uri url = Uri.https(ApiConfig.API_ECOMMECE, '/users/$id');
  ...
  Future<Resource<User>> updateImage(int id, User user, File file) async {
    ...
    try {
      ...
      // http://192.168.80.13:3000/users/5
      Uri url = Uri.https(ApiConfig.API_ECOMMECE, '/users/upload/$id');
      ...
      final response = await request.send(); //funcion qwue envia la peticion
