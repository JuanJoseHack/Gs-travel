...
class AddressService {
  ...
  Future<Resource<Address>> create(Address address) async {
    try {
      Uri url = Uri.https(ApiConfig.API_ECOMMECE, '/address');
  ...
  Future<Resource<List<Address>>> getUserAddress(int idUser) async {
    try {
      Uri url = Uri.https(ApiConfig.API_ECOMMECE, '/address/user/$idUser');
      Map<String, String> headers = {
        "Content-Type": "application/json",
        "Authorization": await token
      };
  ...
  Future<Resource<bool>> delete(int id) async {
    try {
      Uri url = Uri.https(ApiConfig.API_ECOMMECE, '/address/$id');
      Map<String, String> headers = {
        "Content-Type": "application/json",
        "Authorization": await token
      };
